...
# Default configuration file for CYW20829 family of microcontrollers.
...
set_or_global ACQUIRE_TIMEOUT           2500

###############################################################################
# Flash properties and working area
###############################################################################

set_or_global WORKAREAADDR              0x20008000
set_or_global WORKAREASIZE              0x10000
set_or_global QSPI_FLASHLOADER          ../flm/infineon/cyw208xx/CYW208xx_SMIF.FLM

###############################################################################
# Boot settings
###############################################################################
set boot_status_codes {
...
foreach status_code $::boot_status_codes {
	set ::[lindex $status_code 0] [lindex $status_code 1]
}

set app_status_codes {
...
foreach status_code $::app_status_codes {
	set ::[lindex $status_code 0] [lindex $status_code 1]
}
